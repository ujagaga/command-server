...
void mg_list_commands(struct mg_connection *c, char *dir) {
  char path[MG_PATH_MAX], *p = &dir[strlen(dir) - 1];
  struct dirent *dp;
  DIR *dirp;
      ...
      if (snprintf(path, sizeof(path), "%s%s%s", dir, sep, dp->d_name) < 0) {
        ...
        if(end != NULL){
          ...
          if ((fp=fopen(dp->d_name, "r")) != NULL) {
            ...
            // Read one time to skip shebang
            line_size = getline(&buf, &line_buf_size, fp);   
            line_size = 0;
            char comment[256] = {0};

            // Keep reading untill we find # and assume it is the comment
            while(line_size >= 0){
              line_size = getline(&buf, &line_buf_size, fp);
              if((line_size > 0) && (buf[0] == '#')){
                strncpy(comment, &buf[1], sizeof(comment) - 1);
                break;
              }
            }
